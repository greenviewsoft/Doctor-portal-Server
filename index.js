...
// booking details insert  database
  ...
  app.put('/users', async(req, res) => {
   ...
   app.put('/users/admin', verifyToken, async (req, res) => {
      ...
      if(requster){
  const  requsterAccount = await usersCollection.findOne({email: 
    requster});
  if(requsterAccount.role === 'admin'){
    ...
    const filter =  { email: user.email };
       ...
       res.status(403).json({ message: 'you do not have Aceess to this page Thanks'})
   }
